...
def main():
    ...
    # custom-proposal-crm: the PRIMARY CTA is the self-serve CRM trial — refuse to
    # ship without its URL (else the primary button dead-links to a template default).
    # competitorGap is optional (the section auto-hides); when present, its figures
    # must come from the gap-miner (references/rankings.md) — enforced by review.
    _crm_url = (lead.get("crmCta") or {}).get("url")
    if not _crm_url:
        sys.exit("ERROR: crmCta.url is required for the crm variant — the primary CTA is the\n"
                 "  GigRadar CRM trial (https://gigradar.io/crm). Set crmCta in lead-data.json\n"
                 "  (see assets/template-schema.json -> crmCta).")
    ...
    with open(a.template) as f:
        tpl = f.read()
    # expand <!--INCLUDE:relpath--> markers with partial files (resolved next to the
    # template). Lets big self-contained sections (CRM weakness block, trial funnel)
    # live as their own files instead of bloating template.html. Non-recursive.
    tpl_dir = os.path.dirname(os.path.abspath(a.template))
    def _include(m):
        rel = m.group(1).strip()
        p = os.path.join(tpl_dir, rel)
        if not os.path.exists(p):
            print(f"  WARN: INCLUDE not found: {rel}", file=sys.stderr); return ""
        with open(p, encoding="utf-8") as pf:
            return pf.read()
    tpl = re.sub(r"<!--\s*INCLUDE:([^>]+?)-->", _include, tpl)
