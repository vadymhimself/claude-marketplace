...
name: customer-audit
description: >-
  Deep-dive performance audit for a single GigRadar customer team. Retro-first — pulls the client's full Upwork history (pre-GigRadar wins, winning cover letters, positioning), then runs a competitive deep-dive (cohort compare and peer look-alike KNN on the jobs index), then reads chat-room transcripts of hired deals, then builds the Win/Loss cover-letter comparison (paired real winners and losers per scanner with full CL plus JD plus client info side by side), then layers in auto-bidding aggregates as supporting context, and lands a three-tier WINS / OKAY / CRITICAL exec synthesis on sheet 1 of a single dark-mode xlsx workbook. North-star metric is reply rate (not hire rate, since agencies often close off-Upwork). Cost headline is dollars per reply. Use this skill whenever a GM, Growth Manager, or Success Manager asks to audit a specific customer, investigate a poor-performing team, prep a save-the-account call, or pull together a QBR briefing — trigger phrases include "audit team", "why is the team underperforming", "how do we get reply rate up", "what is working for the team", "review the account".
---
