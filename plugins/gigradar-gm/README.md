...
- `0.5.0` — **Scripts are now the default path for `/customer-audit`**, with full env-var parameterization. SKILL.md previously told the agent the scripts were reference-only (`"guidelines, not pre-templated pipeline"`) — so most audits skipped them and hand-rolled the workbook with openpyxl, missing every v0.4.x improvement (freelancer-side enrichment, AI judgment merge, dark-mode 7-sheet builder, OUR WIN labels, the perf hardening). Fixed by (a) rewriting SKILL.md's "How to run this skill" section to make `python3 scripts/phaseN_*.py` the default invocation and reframing creative latitude as interpretation + recommendation copy, and (b) adding a v0.5 env-var contract to every script: `TEAM_OID`, `AUDIT_WORK_DIR`, `FOCUS_START` / `FOCUS_END` / `PRIOR_START` / `PRIOR_END` (sensible defaults: today-30d → today), `AUDIT_OUT_XLSX`. Reading `TEAM_OID` from env eliminates the per-customer sed-editing that previously discouraged script use. Agents retain full creative latitude in (a) WINS/OKAY/CRITICAL tier prose, (b) pattern-spotting across the script-produced tables, (c) recommendation specificity, and (d) edge cases where the data shape genuinely doesn't fit a phase.
